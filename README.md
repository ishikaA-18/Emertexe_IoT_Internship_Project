...
![Simulator](https://img.shields.io/badge/Simulator-PicsimLab-orange)
...
- 🧩 **Modular Code Architecture** — Clean separation across `SensorManager`, `actuator`, `network`, `telemetry` modules
...
```
Emertexe_IoT_Internship_Project/
│
├── node1/
│   ├── node1.ino             # Main sketch (setup + loop)
│   ├── config.h              # Pins, thresholds, MQTT config (⚠️ add token here)
│   ├── SensorManager.h/.cpp  # DHT22, LM35, vibration ADC reads
│   ├── actuator.h/.cpp       # Relay & LED control
│   ├── network.h/.cpp        # Ethernet init, MQTT connect/reconnect
│   └── telemetry.h/.cpp      # JSON payload builder & publisher
│
├── node2/                    # ⏳ Coming soon
│
├── docs/
...
5. Open PicsimLab Serial Terminal — confirm `Connected` message
...
| 6 | Smart Factory — Node 1 (Production Line) | ✅ Complete & Live on ThingsBoard |
| 7 | Smart Factory — Node 2 (Warehouse) | ⏳ Upcoming |
