...
void telemetry_publishTelemetry(const SensorData* data, uint8_t relayState){
  ...
  snprintf_P(buffer,len,PSTR("{\"temperature\":%s,\"humidity\":%s,\"lm35Temp\":%s,\"vibration\":%u,\"machineStatus\":%u,\"relayState\":%u,\"sensorError\":%u}"),t,h,l,
                                (unsigned)data->vibration,(unsigned)data->machineStatus,(unsigned)relayState,(unsigned)data->sensorError);
