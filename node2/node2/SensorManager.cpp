...
static bool readPIR(SensorManager* sm){
  ...
  return sm->motionActive; //1 if there is motion orelse 0
}
//read door status
static bool readDoor(void){
  return (digitalRead(PIN_DOOR)==LOW); //0 IF DOOR IS OPEN,1 DOOR IS CLOSED
}
//read ldr sensor 
static uint16_t readLDR(void){
  ...
  return sum/4;     // warehouse is dark or not if the value is less than 300 else light is available
}
